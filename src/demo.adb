...
begin
   ...
   begin
      ...
      while not Finished loop
         ...
         --  Draw GUI.
         declare
            Title  : C.Strings.chars_ptr := C.Strings.New_String ("Demo");
            Button : C.Strings.chars_ptr := C.Strings.New_String ("button");

            use type C.unsigned;
            use type Nk.nk_flags;
         begin
            if Nk.start (Context, Title, Nk.nk_rect_t'(50.0, 50.0, 230.0, 250.0),
            ...
            then
               ...
               if Nk.button_label (Context, Button) then
                  IO.Put_Line ("Button pressed.");
               end if;

               Nk.layout_row_dynamic (Context, Height => 30.0, Cols => 2);

               C.Strings.Free (Title);
               C.Strings.Free (Button);
            end if;
